...
class ValidadorRegistro {
    ...
    private function validar_nombre($conexion,$nombre) {
        ...
        if (RepositorioUsuario :: nombre_existe($conexion,$nombre)){
            return " este nombre de usuario ya esta en uso, por favor pruebe otro nombre";
        }
        return "";
    }
    ...
    private function validar_email($conexion,$email) {
        if (!$this->variable_iniciada($email)) {
            return "deves proporcionar un email";
        } else {
            $this->email = $email;
        }
        if (RepositorioUsuario :: email_existe($conexion,$email)){
            return ' el email ya esta en uso,pruebe otro por favor<br/> <a href="#"> intente recuperar tu comtraseña </a>.';
        }
        return "";
    }
    ...
    private function validar_clave1($clave1) {
        if (!$this->variable_iniciada($clave1)) {
            return "deves escribir una contraseña";
        }
    ...
    private function validar_clave2($clave1, $clave2) {
        if (!$this->variable_iniciada($clave1)) {
            return "primero deves rrellenar la comtraseña";
        }
        if (!$this->variable_iniciada($clave2)) {
            return"deves repetir tu contraseña";
        }
        if ($clave1 !== $clave2) {
            return"anvas contraseñas deves coincidir";
        }
        return"";
    }
